...
template<typename ValueType, template <typename> typename KeyExtractor> inline typename HOTSingleThreaded<ValueType, KeyExtractor>::const_iterator HOTSingleThreaded<ValueType, KeyExtractor>::findForNonEmptyTrie(typename HOTSingleThreaded<ValueType, KeyExtractor>::KeyType const & searchKey) const {
	...
	/*uint8_t searchByteArray[idx::contenthelpers::MAX_STRING_KEY_LENGTH];
	memset(searchByteArray, 0, sizeof(searchByteArray));*/
	//idx::contenthelpers::interpretAsByteArray(fixedSizedSearchKey, searchByteArray);
  uint8_t const* searchByteArray = idx::contenthelpers::interpretAsByteArray(fixedSizedSearchKey);
  auto start=std::chrono::high_resolution_clock::now();
	PrefetchTable* PTptr=PT;
	size_t PTpos=PTptr->searchPT(searchByteArray);
	size_t match=PTptr->getMatch(searchByteArray,PTpos);
    PTEntry &entry=PTptr->EntryList[PTpos];
    //std::cout<<"call PT in FFNT"<<std::endl;
    PTptr->allcover=PTptr->allcover+1;
    if(match>0)PTptr->cover=PTptr->cover+1;
    //std::cout<<"call cover++ in FFNT"<<std::endl<<"ac="<<PTptr->allcover<<"    cover="<<PTptr->cover<<std::endl;
	for(size_t i=0;i<match;i++)
	{
		HOTSingleThreadedNodeBase const * node = entry.AccessPath[i].getNode();
   //std::cout<<"call PF in search"<<std::endl;
	...
	size_t APlength=0;
	HOTSingleThreadedChildPointer APlist[5];
 auto breakpoint1=std::chrono::high_resolution_clock::now();
	...
	while(!current->isLeaf()) {
		current = it.descend(//current->getNode()->begin(), 
			...
			return node.search(searchByteArray);
		}), current->getNode()->end());
		APlist[APlength]=*current;
		APlength++;
	}
 auto breakpoint2=std::chrono::high_resolution_clock::now();
 size_t hits_inthis=0;
  for(size_t i=0;i<match;i++)
  {
    PTptr->allhit=PTptr->allhit+1;
    if(entry.AccessPath[i]==APlist[i]) hits_inthis=hits_inthis+1;
     //std::cout<<"call hit++ in FFNT"<<std::endl<<"ah="<<PTptr->allhit<<"    hit="<<PTptr->hit<<std::endl;
     //std::cout<<"verify hit++ in FFNT"<<std::endl<<"ah="<<PT->allhit<<"    hit="<<PT->hit<<std::endl;
  }
  PT->hit=PT->hit+hits_inthis;
	if(match==0 || hits_inthis==0)
	{
		PTptr->insertPT(searchByteArray,APlength,APlist);
	}
auto breakpoint3=std::chrono::high_resolution_clock::now();
auto st=std::chrono::duration_cast<std::chrono::nanoseconds>(breakpoint1-start);
auto rt=std::chrono::duration_cast<std::chrono::nanoseconds>(breakpoint2-breakpoint1);
auto ut=std::chrono::duration_cast<std::chrono::nanoseconds>(breakpoint3-breakpoint2);
//std::cout<<"searchtime="<<(double)(st.count())<<std::endl;
//std::cout<<"old avg searchtime="<<PTptr->search_time<<std::endl;
PTptr->find_count=PTptr->find_count+1;
PTptr->search_time=PTptr->search_time+((double)(st.count())-PTptr->search_time)/PTptr->find_count;
//std::cout<<"new avg searchtime="<<PTptr->search_time<<std::endl;
PTptr->read_time=PTptr->read_time+((double)(rt.count())-PTptr->read_time)/PTptr->find_count;
PTptr->update_time=PTptr->update_time+((double)(ut.count())-PTptr->update_time)/PTptr->find_count;
...
template<typename NodeType> inline void insertNewValueResultingInNewPartitionRoot(NodeType const &existingNode,
   ...
   //std::cout<<"in insertNewValueResultingInNewPartitionRoot"<<std::endl<<std::endl;
		delete &existingNode;
	} else {
...
template<typename NodeType> inline void insertNewValue(NodeType &existingNode,
													   std::array<HOTSingleThreadedInsertStackEntry, 64> &insertStack,
...
/*
	if (!existingNode.isFull()) {
		...
		size_t newentry=existingNode.getNumberEntries();
		if(newentry==1 || newentry==16)
		{
		  HOTSingleThreadedChildPointer newNodePointer = existingNode.addEntry(insertInformation, valueToInsert);
        std::cout<<"New node addr= "<<newNodePointer.getNode()<<std::endl;
		...
		HOTSingleThreadedChildPointer newNodePointer = existingNode.addstaticEntry(insertInformation, valueToInsert);
   *(insertStackEntry.mChildPointer) = newNodePointer;
   std::cout<<"New node addr,but no alloc= "<<newNodePointer.getNode()<<std::endl;
		/*	newNodePointer.executeForSpecificNodeType(false, [&](auto & node) {
				std::memmove((void*)&existingNode,(void*)&node,sizeof(node));
			});
			std::cout<<"existingNode addr= "<<&existingNode<<std::endl;
		}
	} else {
...
template<typename ValueType, template <typename> typename KeyExtractor> inline void HOTSingleThreaded<ValueType, KeyExtractor>::collectStatsForSubtree(HOTSingleThreadedChildPointer const & subTreeRoot, std::map<std::string, double> & stats) const {
	if(!subTreeRoot.isLeaf()) {
      ...
      std::string heightName = "Height="+std::to_string(node.mHeight);
      std::string heightsizeName = "size@level"+std::to_string(node.mHeight);
      stats[heightName] += 1.0;
      stats[heightsizeName] += node.getNodeSizeInBytes();
      std::string numberName = "NumEntry_innode="+std::to_string(node.getNumberEntries());
      stats[numberName] += 1.0;
...
template<typename ValueType, template <typename> typename KeyExtractor> std::pair<size_t, std::map<std::string, double>> HOTSingleThreaded<ValueType, KeyExtractor>::getStatistics() const {
 ...
 statistics["numberNode allocate"] = HOTSingleThreadedNodeBase::getNumberNode();
 ...
 std::cout<<"ask cover in stat"<<std::endl<<"ac="<<PT->allcover<<"    cover="<<PT->cover<<std::endl;
 std::cout<<"ask hit in stat"<<std::endl<<"ah="<<PT->allhit<<"    hit="<<PT->hit<<std::endl;
 statistics["p_allcover"]=PT->allcover;
 statistics["p_cover"]=PT->cover;
 statistics["p_allhit"]=PT->allhit;
 statistics["p_hit"]=PT->hit;
 statistics["p_cover ratio"]=((double)PT->cover)/((double)PT->allcover);
 statistics["p_hit ratio"]=((double)PT->hit)/((double)PT->allhit);
 statistics["ptime_search"]=PT->search_time;
 statistics["ptime_read"]=PT->read_time;
 statistics["ptime_update"]=PT->update_time;
 statistics["ptime_count"]=PT->find_count;
