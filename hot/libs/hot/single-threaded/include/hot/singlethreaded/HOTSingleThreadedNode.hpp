...
template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void* HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType>::operator new (size_t /* baseSize */, uint16_t const numberEntries, void* defaultptr) {
	size_t AllocEntry=16;
	if(numberEntries>16)AllocEntry=32;
	hot::commons::NodeAllocationInformation const & allocationInformation = hot::commons::NodeAllocationInformations<HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType>>::getAllocationInformation(AllocEntry);
	...
	void* memory;
	if(numberEntries==2 || numberEntries==17)
	{
  std::cout<<"try alloc, size= "<<sizeInBytes<<"  AllocEntry= "<<AllocEntry<<std::endl;
		memory = HOTSingleThreadedNodeBase::getMemoryPool()->alloc(sizeInBytes/sizeof(uint64_t));
		std::cout<<"new alloc, return addr= "<<memory<<std::endl;
	}
	else
	{
		memory = defaultptr;
		std::cout<<"static alloc, return addr= "<<memory<<std::endl;
	}
	return memory;
...
) const
{
	...
	/*uint16_t newAllocEntries = 2;
	if(newNumberEntries>16) newAllocEntries=17;*/
	//std::cout<<"New Number Entries= "<<newNumberEntries<<std::endl;
	//_D(k2::log::hot, "New Number Entries={}", newNumberEntries);
...
template<typename DiscriminativeBitsRepresentation, typename PartialKeyType>  inline
HOTSingleThreadedChildPointer HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType>::addstaticEntry(
	hot::commons::InsertInformation const & insertInformation, HOTSingleThreadedChildPointer const & newValue
)
{
	uint16_t newNumberEntries = getNumberEntries() + 1;
	HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType> & self = *this;
 HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType> * swaparea=(HOTSingleThreadedNode<DiscriminativeBitsRepresentation, PartialKeyType> *) HOTSingleThreadedNodeBase::getMemoryPool()->allocstatic();
 std::memmove((void*)swaparea,(void*)&self,self.getNodeSizeInBytes());
 swaparea->mFirstChildPointer=reinterpret_cast<HOTSingleThreadedChildPointer*>(reinterpret_cast<char*>(self.mFirstChildPointer) - reinterpret_cast<char*>(&self) +reinterpret_cast<char*>(swaparea));
 std::cout<<self.mFirstChildPointer<<std::endl<<&self<<std::endl<<swaparea<<std::endl<<swaparea->mFirstChildPointer<<std::endl;
//TODO:need deep copy
	return mDiscriminativeBitsRepresentation.insert(insertInformation.mKeyInformation, [&](auto const & newDiscriminativeBitsRepresentation)  {
		using NewConstDiscriminativeBitsRepresentationType = typename std::remove_reference<decltype(newDiscriminativeBitsRepresentation)>::type;
		using IntermediateNewDiscriminativeBitsRepresentationType = typename std::remove_const<NewConstDiscriminativeBitsRepresentationType>::type;
		typedef typename ToDiscriminativeBitsRepresentation<IntermediateNewDiscriminativeBitsRepresentationType, PartialKeyType>::Type NewDiscriminativeBitsRepresentationType;
		return (newDiscriminativeBitsRepresentation.calculateNumberBitsUsed() <= (sizeof(PartialKeyType) * 8))
			? (new (newNumberEntries,&self) HOTSingleThreadedNode<NewDiscriminativeBitsRepresentationType, typename ToPartialKeyType<NewDiscriminativeBitsRepresentationType, PartialKeyType>::Type>(
				*swaparea, newNumberEntries, newDiscriminativeBitsRepresentation, insertInformation, newValue
			  ))->toChildPointer()
			: (new (newNumberEntries,&self) HOTSingleThreadedNode<
					typename ToDiscriminativeBitsRepresentation<NewDiscriminativeBitsRepresentationType, typename NextPartialKeyType<PartialKeyType>::Type>::Type,
					typename ToPartialKeyType<NewDiscriminativeBitsRepresentationType,typename NextPartialKeyType<PartialKeyType>::Type>::Type
				> (
				*swaparea, newNumberEntries, newDiscriminativeBitsRepresentation, insertInformation, newValue
