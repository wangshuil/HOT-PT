...
#define PT_MAX_SIZE (8192)
#define PT_SEARCH_HASH_BYTES (2)
...
/*
64bytes entry for PT
|<---16Bytes--->|<--8Bytes-->|<---5*8Bytes--->|
|<---Key    --->|<--flags -->|<---5*pointer--->|
AP: array of HOTSingleThreadedChildPointer,use getnode to extract address
Flags:
|1bit|2bits         |2bits         |3bits         |<---56bits---> |
|full|err counter #1|err counter #2|AP length     |key length|
*/
class PTEntry
{
public:
	uint8_t KeySlice[16];
	uint64_t flags;
	hot::singlethreaded::HOTSingleThreadedChildPointer AccessPath[5];
	...
	PTEntry(uint8_t const* KeyByteArray,size_t APlength,hot::singlethreaded::HOTSingleThreadedChildPointer* APlist)
	{
		for(int i=0;i<16;i++)KeySlice[i]=KeyByteArray[i];
		for(int i=0;i<APlength && i<5 ;i++)
		{
			AccessPath[i]=APlist[i];
		}
		flags=0;
	}
...
{
public:
	PTEntry EntryList[PT_MAX_SIZE];
	std::hash<uint64_t> hash_64;
   uint64_t allcover,cover,allhit,hit;
   double search_time,read_time,update_time;
   int find_count;
	PrefetchTable()
	{
		memset(EntryList,0,sizeof(EntryList));
   std::cout<<"PT entry number= "<<PT_MAX_SIZE<<std::endl<<"PT size= "<<sizeof(EntryList)<<std::endl;
   allcover=0;
   cover=0;
   hit=0;
   allhit=0;
   ...
   find_count=0;
	...
	int insertPT(uint8_t const* KeyByteArray,size_t APlength,hot::singlethreaded::HOTSingleThreadedChildPointer* APlist)
	{
		size_t pos;
		uint64_t Keyidx=0;
		for(int i=0;i<PT_SEARCH_HASH_BYTES;i++)
		{
			Keyidx=Keyidx<<8;
			Keyidx=Keyidx | KeyByteArray[i];
		}
		pos=(hash_64(Keyidx) % PT_MAX_SIZE);
		EntryList[pos]=PTEntry(KeyByteArray,APlength,APlist);
		return 0;
	}
	size_t searchPT(uint8_t const* KeyByteArray)
	{
		size_t pos;
		uint64_t Keyidx=0;
		for(int i=0;i<PT_SEARCH_HASH_BYTES;i++)
		{
			Keyidx<<=8;
			Keyidx |= KeyByteArray[i];
		}
		pos=(hash_64(Keyidx) % PT_MAX_SIZE);
		return pos;
	}
	size_t getMatch(uint8_t const* KeyByteArray,size_t pos)
	{
		size_t i=0;
		for(i=0;i<16;i++)
		{
			if(KeyByteArray[i]==EntryList[pos].KeySlice[i]) continue;
			break;
		}
   if(i==0)return 0;
   if(i>5)i=5;
...
template<typename NodeType> inline void insertNewValue(
	NodeType &existingNode, std::array<HOTSingleThreadedInsertStackEntry, 64> &insertStack,
