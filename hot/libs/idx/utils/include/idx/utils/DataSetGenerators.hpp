...
std::vector<uint64_t> createRandomDataSet(size_t size, bool isVerbose=false) {
	...
	size_t all_size=size*1.5;
	//std::vector<uint64_t> results(all_size);
	std::cout << "createRandomDataSet, all size=" << all_size<<std::endl;
	if(isVerbose) {
		std::cout << "Using seed:" << rnd.getSeed() << std::endl;
	}

	while(uniqueRandomValues.size() < all_size) {
		uniqueRandomValues.insert(rnd());
	}
 std::cout << "createRandomDataSet Finish, all size=" << uniqueRandomValues.size()<<std::endl;
