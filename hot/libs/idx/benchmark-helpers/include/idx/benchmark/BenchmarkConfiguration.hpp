...
public:
	...
	std::shared_ptr<std::vector<uint64_t>> mWriteKeys;
	...
	BenchmarkConfiguration(idx::utils::CommandParser const & params)
			...
			, mWriteKeys(new std::vector<uint64_t>)
   ...
   std::vector<uint64_t> tempvector = idx::utils::creatDataSet(params, "insert");
		size_t realsize=params.expect<size_t>("size");
		(*mWriteKeys).assign(tempvector.begin()+realsize,tempvector.end());
		(*mInsertKeys).assign(tempvector.begin(),tempvector.begin()+realsize);
	...
	std::vector<uint64_t> & getWriteValues() {
		return *mWriteKeys;
	}
