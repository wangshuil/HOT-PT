...
public:
	...
	int run() {
		...
		if(!mBenchmarkConfiguration.isInsertOnly()) {
			...
			std::vector<uint64_t> const &writeKeys = mBenchmarkConfiguration.getWriteValues();
			size_t totalNumberLookups = 100000000;

			benchmarkOperation("lookup", lookupKeys, totalNumberLookups+writeKeys.size(), [&](std::vector<uint64_t> const &keys) {
					size_t totalNumberKeys = lookupKeys.size();
          size_t totalwrites = writeKeys.size();
					int wrratio=totalNumberKeys/totalwrites;
					std::cout << "wrratio="<< wrratio<<"   totalwrites="<<totalwrites<<std::endl;
						...
						return boost::hana::if_(hasThreadInfo(mBenchmarkable),
																		...
																		[&](auto &benchmarkable) {
																				...
																				for (size_t i = 0; i < totalNumberLookups; ++i) {
																					...
																					if(index % wrratio == 0 && w_index<totalwrites)
                                          {
                                          benchmarkable.insert(writeKeys[w_index]);
																					++w_index;
                                          }
																					++index;
																				}
                                        std::cout << "w_index="<< w_index<<"   totalwrites="<<totalwrites<<std::endl;
																				return allLookedUp;
																		}
						)(mBenchmarkable);
					}
